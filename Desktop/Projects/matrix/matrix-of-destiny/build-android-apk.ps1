### Matrix of Soul — Android Release APK (for local testing) ###
# Те саме що build-android.ps1 але APK замість AAB — можна ставити на телефон через adb
...
Write-Host "=== STEP 10: Build APK ===" -ForegroundColor Cyan
Set-Location "$BUILD_DIR\android"
.\gradlew assembleRelease

Write-Host ""
Write-Host "=== BUILD COMPLETE ===" -ForegroundColor Green
$apkPath = "$BUILD_DIR\android\app\build\outputs\apk\release\app-release.apk"
Write-Host "APK: $apkPath" -ForegroundColor Yellow
Write-Host ""
Write-Host "Install on phone: adb install $apkPath" -ForegroundColor Yellow
