...
// Apple reviewer account — always returns premium state (no DB needed)
const REVIEWER_UID = 'bhJDbGO7eESYkDKO68qW3xkzeEg1';
const REVIEWER_STATE = {
  isPremium: true, plan: 'yearly', isAuthenticated: true,
  userId: REVIEWER_UID, userName: 'Apple Reviewer',
  tokens: 999, streak: 5, level: 5, xp: 500, onboardingCompleted: true,
};
...
app.get('/api/sync/:userId', verifyToken, async (req, res) => {
  ...
  if (req.params.userId === REVIEWER_UID) return res.json({ state: REVIEWER_STATE });
  try {
